...
export default function ShortTermGoalsPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedProgress) {
        ...
        if (progress.shortTermGoals) {
          setScore(progress.shortTermGoals)
          setIsCompleted(progress.shortTermGoals >= 2.5)
        }
      }
    } catch (error) {
  ...
  const handleTaskComplete = (nextPhase?: string) => {
    ...
    try {
      ...
      progress.shortTermGoals = scoreValue
  ...
  const continueToNextTask = () => {
    router.push('/phase4/contingency-strategies')
  }
          ...
          <Card className="bg-gray-900/60 backdrop-blur-md border border-orange-500/30 shadow-xl mb-6">
            ...
            <CardContent>
              <div className="text-white/80 space-y-4 mb-4">
                <p>
                  Create SMART objectives that are Specific, Measurable, Achievable, Relevant, and Time-bound to break down your long-term goals into manageable steps.
                </p>
              </div>
