    ...
    <div className="space-y-2 w-full">
      {sections.map((section, index) => {
        ...
        return (
          ...
          >
            <h3 className="font-semibold text-sm mb-1 flex items-center break-words" style={{ color: accent }}>
              {title === "Assessment" && <BarChart className="w-3.5 h-3.5 mr-1.5 flex-shrink-0" />}
              {title === "Guidance" && <Sparkles className="w-3.5 h-3.5 mr-1.5 flex-shrink-0" />}
              {title === "Next Steps" && <ArrowRight className="w-3.5 h-3.5 mr-1.5 flex-shrink-0" />}
              {title}
            </h3>
            <div className="text-xs break-words overflow-wrap-anywhere max-w-full overflow-hidden" style={{ color: "hsl(var(--foreground))" }}>
